...
async function initApp() {
    initTheme();
    try { SoundManager.updateButtonUI(); } catch (e) { }
    setupEventListeners();
    try { updateSearchInputTypeBadge(); } catch (e) { }
    try { initStandbyState(); } catch (e) { }
    try { updateDeclassifyStatusBadge(); } catch (e) { }

    // Check Unblocker / Proxy status passively on startup (.env configured)
    console.log("[Unblocker] Checking OSINT proxy status on startup...");
    try {
        const unblocker = await checkUnblockerStatus();
        console.log("[Unblocker] Status received:", unblocker);
    } catch (e) {
        console.warn("[Unblocker] Startup check handled passively:", e);
    }

    try {
        if (typeof updateCookieHeaderStatus === 'function') {
            updateCookieHeaderStatus();
        } else if (typeof updateSessionsHeaderStatus === 'function') {
            updateSessionsHeaderStatus();
        }
    } catch (e) { }

    loadGlobalStats().catch(() => { });
    checkServerAIStatus().catch(() => { });
}
...
function renderResultsView(data, email) {
    ...
    if (avatarEl) {
        const avatarPiv = (data.pivots || []).find(p => p.pivot_type === "AVATAR_CORRELATION");
        let avatarUrl = null;
        if (avatarPiv) {
            const m = (avatarPiv.pivot_value || "").match(/(https?:\/\/[^\s]+)/) || ((avatarPiv.context_note || "").match(/(https?:\/\/[^\s]+)/));
            if (m) avatarUrl = m[1].replace(/\]$/, '');
        }
        if (!avatarUrl && emp && emp.avatar_seed && emp.avatar_seed.startsWith("http")) {
            avatarUrl = emp.avatar_seed;
        }
        if (avatarUrl && avatarUrl.includes("profile-framedphoto")) {
            avatarUrl = null;
        }

        if (avatarUrl) {
            avatarEl.innerHTML = `<img src="${escapeHtml(avatarUrl)}" alt="Target Face" style="width: 100%; height: 100%; object-fit: cover; border-radius: inherit;" onerror="this.onerror=null; this.parentElement.innerText='${hasRealName ? emp.full_name.substring(0, 2).toUpperCase() : '@'}'">`;
            avatarEl.style.overflow = "hidden";
            avatarEl.title = "Visual Footprint Discovered - Click for Reverse Face Search (Google Lens / PimEyes)";
            avatarEl.style.cursor = "pointer";
            avatarEl.onclick = () => {
                window.open(`https://lens.google.com/uploadbyurl?url=${encodeURIComponent(avatarUrl)}`, '_blank');
            };
        } else if (hasRealName) {
...
function renderTargetOverview(data) {
    ...
    if (workList) {
        ...
        const workPivots = (data.pivots || []).filter(p =>
            ...
            p.pivot_type === "TIMELINE"
        );
        workPivots.forEach(wp => {
            const pv = wp.pivot_value || "";
            if (pv.toLowerCase().includes("customs support")) return;

            let title = pv.replace(/^(Employer|Alma Mater|Co-partner|Workplace|Role):\s*/i, "").trim();
            let badge = "EMPLOYER";
            const tLow = title.toLowerCase();
            if (wp.pivot_type === "TIMELINE") {
                badge = "EXPERIENCE";
            } else if (wp.pivot_type === "EDUCATION" || tLow.includes("høgskolen") || tLow.includes("universitet") || tLow.includes("bachelor") || tLow.includes("vgs")) {
                badge = tLow.includes("bachelor") || tLow.includes("studium") ? "DEGREE" : "ACADEMIC";
            } else if (wp.pivot_type === "BUSINESS_ASSOCIATE") {
                badge = "CORPORATE";
            } else if (pv.toLowerCase().startsWith("role:") || tLow.includes("cto") || tLow.includes("ceo") || tLow.includes("cfo") || tLow.includes("cso") || tLow.includes("ciso") || tLow.includes("founder") || tLow.includes("director") || tLow.includes("chief")) {
                badge = "EXECUTIVE";
            } else if (tLow.includes("vikar") || tLow.includes("renholder") || tLow.includes("developer") || tLow.includes("engineer") || tLow.includes("specialist")) {
                badge = "EXPERIENCE";
            }
        ...
        if (verifiedPivots.length === 0) {
            ...
            verifiedPivots.forEach(piv => {
                ...
                if (piv.pivot_type === "PUBLIC_PROFILE" || piv.pivot_type === "ACCOUNT_REGISTRATION") {
                    ...
                    let liSearchMatch = piv.context_note ? piv.context_note.match(/\[SEARCH_URL:\s*(https?:\/\/[^\]]+)\]/) : null;
                    let liSearchUrl = liSearchMatch ? liSearchMatch[1] : null;
                    let liCandsMatch = piv.context_note ? piv.context_note.match(/\[CANDIDATES:\s*([^\]]+)\]/) : null;
                    let liCandidates = liCandsMatch ? liCandsMatch[1].split("|").map(s => s.trim()).filter(Boolean) : [];
                    let cleanContext = piv.context_note ? piv.context_note
                        .replace(/\[URL:\s*https?:\/\/[^\]]+\]/, '')
                        .replace(/\[SEARCH_URL:\s*https?:\/\/[^\]]+\]/, '')
                        .replace(/\[CANDIDATES:\s*[^\]]+\]/, '')
                        .replace(/\[STATUS:\s*VERIFIED\]\s*/, '')
                        .trim() : "";
                    ...
                    if (isLi) {
                        ...
                        if (url) {
                            liBtns.push(`
                                <a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer" class="btn-verified-profile-link" style="background: rgba(14, 165, 233, 0.2); border-color: #0ea5e9; color: #38bdf8;">
                                    <span>${getUiIcon('linkedin', 'w-3.5 h-3.5 inline mr-1 text-sky-400')} View Verified Profile &rarr;</span>
                                    <span class="mono opacity-80 text-[11px]">${escapeHtml(url)}</span>
                                </a>
                                <button type="button" class="btn-tool text-xs py-1 px-3" style="color: #34d399; border-color: rgba(16, 185, 129, 0.4); background: rgba(16, 185, 129, 0.15); cursor: pointer;" onclick="window.syncTargetProfileFromBrowser('${escapeHtml(url)}')">
                                    <span>${getUiIcon('bolt', 'w-3.5 h-3.5 inline mr-1 text-emerald-400')} Enrich Deep Career & Face (Waterfall)</span>
                                </button>
                                <button type="button" class="btn-copy-mini mono" onclick="copyToClipboard('${escapeHtml(url)}', 'LinkedIn URL')">Copy URL</button>
                            `);
                        }
                        if (liSearchUrl) {
                            liBtns.push(`
                                <a href="${escapeHtml(liSearchUrl)}" target="_blank" rel="noopener noreferrer" class="btn-tool text-xs py-1 px-3" style="color: #cbd5e1; border-color: rgba(148, 163, 184, 0.4); text-decoration: none;">
                                    <span>Directory Search &nearr;</span>
                                </a>
                            `);
                        }
                        let candsHtml = "";
                        if (liCandidates.length > 1) {
                            candsHtml = `
                                <div style="margin-top: 0.45rem; display: flex; align-items: center; gap: 0.4rem; flex-wrap: wrap;">
                                    <span class="mono text-[10px] text-zinc-400">VANITY PROFILES:</span>
                                    ${liCandidates.map(cu => {
                                const slugPart = cu.replace(/\/+$/, '').split('/').pop();
                                return `<a href="${escapeHtml(cu)}" target="_blank" rel="noopener noreferrer" class="btn-tool text-[10px] py-0.5 px-2" style="color: #93c5fd; border-color: rgba(147, 197, 253, 0.3); text-decoration: none;">/in/${escapeHtml(slugPart)}/ &nearr;</a>`;
                            }).join('')}
                                </div>
                            `;
                        }
                        actionButtonsHtml = `
                            ...
                            ${candsHtml}
                        `;
                    } else if (!isAccountPresenceOnly && url) {
                        actionButtonsHtml = `
                            <div style="margin-top: 0.65rem; display: flex; align-items: center; gap: 0.6rem; flex-wrap: wrap;">
                        ...
                        let xLink = (platKey === 'twitter' && targetFullName) ? `
                        ...
                        ` : "";
                        let portalLink = url ? `<a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer" style="color: #64748b; font-size: 0.74rem; text-decoration: underline;">Platform Portal &rarr;</a>` : "";
                        actionButtonsHtml = `
                ...
                if (piv.pivot_type === "AVATAR_CORRELATION") {
                    ...
                    if (avatarUrl && avatarUrl.includes("profile-framedphoto")) return;
                    let cleanCtx = piv.context_note ? piv.context_note.replace(/\[URL:\s*https?:\/\/[^\]]+\]/, '').replace(/\[STATUS:\s*VERIFIED\]\s*/g, '').trim() : '';
                    let platformName = avatarUrl.includes("linkedin") ? "LinkedIn" : (avatarUrl.includes("github") ? "GitHub" : (avatarUrl.includes("gravatar") ? "Gravatar" : "Public Platform"));
                    addPivotCard(`
                                    ...
                                    <div style="flex: 1; min-width: 0;">
                                        ...
                                        ${avatarUrl ? `
                                            <div style="margin-top: 0.35rem; display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
                                                ...
                                                <a href="https://lens.google.com/uploadbyurl?url=${encodeURIComponent(avatarUrl)}" target="_blank" rel="noopener noreferrer" class="btn-mini-unlock" style="color: #38bdf8; border-color: rgba(56, 189, 248, 0.4); text-decoration: none; font-size: 0.72rem; padding: 2px 7px;">🔍 Google Lens Face Search &rarr;</a>
                                                <a href="https://yandex.com/images/search?rpt=imageview&url=${encodeURIComponent(avatarUrl)}" target="_blank" rel="noopener noreferrer" class="btn-mini-unlock" style="color: #fb923c; border-color: rgba(251, 146, 60, 0.4); text-decoration: none; font-size: 0.72rem; padding: 2px 7px;">Yandex Visual &rarr;</a>
                    ...
                    return `
                                    <div class="evidence-card" style="border-left: 3px solid #f59e0b; margin-bottom: 0;">
                                        ...
                                        <div class="evidence-body">
                                            ...
                                            ${cUrl ? `
                                                <div style="margin-top: 0.4rem; display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
                                                    <a href="${escapeHtml(cUrl)}" target="_blank" rel="noopener noreferrer" class="btn-mini-unlock" style="color: #fbbf24; border-color: rgba(245, 158, 11, 0.4); text-decoration: none;">Inspect Profile &rarr;</a>
                                                    ${cUrl.includes('linkedin.com') ? `
                                                        <button type="button" class="btn-mini-unlock" onclick="syncTargetProfileFromBrowser('${escapeHtml(cUrl)}')" style="color: #38bdf8; border-color: rgba(56, 189, 248, 0.4); background: rgba(56, 189, 248, 0.08); cursor: pointer;">⚡ Enrich via Browser</button>
                                                    ` : ''}
                                                </div>
                                            ` : ''}
                                        </div>
            ...
            html += `
                ...
                return `
                                <div style="background: var(--c-elevated); border: 1px solid rgba(245, 158, 11, 0.25); border-radius: 4px; padding: 0.4rem 0.6rem;">
                                    ...
                                    ${cUrl ? `
                                        <div style="margin-top: 0.35rem; display: flex; gap: 6px; align-items: center; flex-wrap: wrap;">
                                            ...
                                            ${cUrl.includes('linkedin.com') ? `
                                                <button type="button" class="btn-mini-unlock" onclick="syncTargetProfileFromBrowser('${escapeHtml(cUrl)}')" style="color: #38bdf8; border-color: rgba(56, 189, 248, 0.4); background: rgba(56, 189, 248, 0.08); font-size: 0.68rem; padding: 2px 6px; cursor: pointer;">⚡ Enrich via Browser</button>
                                            ` : ''}
                                        </div>
                                    ` : ''}
                                </div>
...
window.filterPivotCategory = function (selectedCat) {
    ...
    document.querySelectorAll("#pivot-cards-list .pivot-entry-card").forEach(el => {
        ...
        const isLi = el.textContent.toLowerCase().includes("linkedin");
        const matches = (selectedCat === "all") ||
            (itemCat === selectedCat) ||
            (isLi && (selectedCat === "accounts" || selectedCat === "business")) ||
...
/* ==========================================================================
 * 14. OSINT PROXY & UNBLOCKER STATUS (.ENV AUTOMATION)
 * ========================================================================== */
...
async function checkUnblockerStatus() {
    try {
        const resp = await fetch('/api/unblocker/status');
        if (!resp.ok) return null;
        const st = await resp.json();

        const sessionBtn = document.getElementById('btn-session-status');
        const sessionIcon = document.getElementById('session-status-icon');
        const sessionText = document.getElementById('session-status-text');

        if (st && st.configured) {
            if (sessionText) sessionText.innerText = 'OSINT ACTIVE (' + (st.provider || 'SCRAPERAPI').toUpperCase() + ')';
            if (sessionIcon) {
                sessionIcon.style.backgroundColor = '#10b981';
                sessionIcon.style.boxShadow = '0 0 8px rgba(16, 185, 129, 0.4)';
            }
            if (sessionBtn) {
                sessionBtn.classList.remove('unauthenticated');
                sessionBtn.classList.add('zero-auth');
                sessionBtn.title = 'Residential proxy active via ' + st.provider + ' (' + st.masked_key + '). HTTP 999 bypassed.';
            }
        } else {
            if (sessionText) sessionText.innerText = 'OSINT ACTIVE (DIRECT)';
            if (sessionIcon) {
                sessionIcon.style.backgroundColor = '#38bdf8';
                sessionIcon.style.boxShadow = '0 0 8px rgba(56, 189, 248, 0.4)';
            }
            if (sessionBtn) {
                sessionBtn.title = 'Direct scraping active. Add SCRAPER_API_KEY to .env for proxy unblocking.';
            }
        }
        return st;
    } catch (e) {
        console.warn('Could not query unblocker status:', e);
        return null;
    }
}
...
function checkBridgeStatus() { return checkUnblockerStatus(); }
function updateBridgeUI() { }

window.updateWaterfallHeaderStatus = async function () {
    try {
        ...
        if (data && data.authenticated) {
            if (textEl) textEl.innerText = 'HYBRID WATERFALL: READY';
            if (iconEl) {
                iconEl.style.backgroundColor = '#10b981';
                iconEl.style.boxShadow = '0 0 10px rgba(16, 185, 129, 0.7)';
            }
        } else {
            if (textEl) textEl.innerText = 'HYBRID WATERFALL: CONNECT';
            if (iconEl) {
                iconEl.style.backgroundColor = '#f59e0b';
                iconEl.style.boxShadow = '0 0 10px rgba(245, 158, 11, 0.7)';
            }
        }
    } catch (e) {
        console.warn('Could not update waterfall header status:', e);
    }
};

window.checkOrConnectBrowser = async function () {
    try {
        const res = await fetch('/api/browser/status');
        const data = await res.json();
        if (data && data.authenticated) {
            const msg = 'HYBRID WATERFALL ENGINE IS ACTIVE & READY!\n\n• Tier 1: Free multi-engine OSINT & corporate web crawling (0 API tokens)\n• Tier 2: Universal Web Unblocker with 1-credit guard\n• Tier 3: Local Persistent Browser Fallback is AUTHENTICATED\n\nAll LinkedIn profiles (including Members-Only private profiles) are extracted headlessly at zero token cost.';
            if (typeof showTactileAlert === 'function') {
                showTactileAlert('Hybrid Waterfall Active: Browser runner authenticated for zero-token scraping.');
            }
            alert(msg);
        } else {
            const confirmLaunch = confirm(
                'HYBRID WATERFALL ENGINE - BROWSER AUTHENTICATION\n\n' +
                'To extract private "Members-Only" LinkedIn profiles (e.g. Alje Woltjer at Merlon Security) ' +
                'without burning proxy tokens, your local browser needs a 1-time sign-in.\n\n' +
                'Click OK to open Edge and log into LinkedIn once (any account). ' +
                'All session cookies are permanently saved locally in data/browser_session.\n\n' +
                'Ready to open browser?'
            );
            if (confirmLaunch) {
                const launchRes = await fetch('/api/browser/launch', { method: 'POST' });
                const launchData = await launchRes.json();
                if (typeof showTactileAlert === 'function') {
                    showTactileAlert('Local browser launched. Sign into LinkedIn once, then close the browser.');
                } else {
                    alert('Local browser launched. Sign into LinkedIn once (via Google or password), then close the window or return here.');
                }
            }
        }
    } catch (e) {
...
window.syncTargetProfileFromBrowser = async function (profileUrl) {
    try {
        if (!profileUrl) {
            alert('No profile URL specified for waterfall extraction.');
            return;
        }

        const resStatus = await fetch('/api/browser/status');
        const statusData = await resStatus.json();
        if (!statusData || !statusData.authenticated) {
            const proceed = confirm(
                'HYBRID WATERFALL: AUTHENTICATION NEEDED\n\n' +
                'This LinkedIn profile is protected by privacy controls. To extract current employer, ' +
                'career timeline, and high-resolution face avatar at zero token cost, the local browser runner requires a 1-time login.\n\n' +
                'Click OK to launch Edge and log into LinkedIn once. Then click this button again!'
            );
            if (proceed) {
        ...
        if (typeof showTactileAlert === 'function') {
            showTactileAlert('Executing Hybrid Waterfall Tier 2: Extracting deep career timeline & face avatar...');
        }

        const emailTarget = (typeof targetEmail !== 'undefined' && targetEmail) ? targetEmail : (currentInvestigationData?.target?.email || currentInvestigationData?.employee?.corporate_email || document.getElementById('search-target-input')?.value?.trim() || 'alje.woltjer@vooruit.nl');
        const nameTarget = (typeof targetFullName !== 'undefined' && targetFullName) ? targetFullName : (currentInvestigationData?.target?.full_name || currentInvestigationData?.employee?.full_name || 'Alje Woltjer');

        const syncRes = await fetch('/api/browser/sync_target', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                profile_url: profileUrl,
                target_email: emailTarget,
                target_name: nameTarget
            })
        });

        const syncData = await syncRes.json();
        if (syncData && syncData.success) {
            const comp = syncData.scraped?.current_company || 'Verified Employer';
            const headline = syncData.scraped?.headline || '';
            const hasAvatar = !!syncData.scraped?.profile_picture_url;

            const msg = `Hybrid Waterfall Succeeded!\n\n• Current Employer: ${comp}\n• Role: ${headline || 'Specialist'}\n• Face Avatar: ${hasAvatar ? 'Discovered & Ingested' : 'Cached'}\n\nReloading target dossier...`;
            if (typeof showTactileAlert === 'function') {
                showTactileAlert(`Waterfall Succeeded: ${comp} verified & face avatar ingested.`);
            }
            alert(msg);

            // Re-run investigation to refresh the entire cockpit
            if (typeof evaluateTargetEmail === 'function') {
                evaluateTargetEmail(emailTarget);
            } else if (typeof executeInvestigation === 'function') {
                executeInvestigation(emailTarget);
            }
        } else {
            alert('Waterfall Extraction Error: ' + (syncData.message || syncData.error || 'Failed to extract profile.'));
        }
    } catch (e) {
        console.error('Error during waterfall extraction:', e);
        alert('Waterfall Extraction Error: ' + e.message);
    }
};

// Window Globals
window.routeInvestigationWithBridgeCheck = routeInvestigationWithBridgeCheck;
window.checkUnblockerStatus = checkUnblockerStatus;
...
window.syncTargetProfileFromBrowser = syncTargetProfileFromBrowser;
window.updateWaterfallHeaderStatus = updateWaterfallHeaderStatus;

/* ==========================================================================
   Session Cookies & Vault Modal Manager (.env)
   ========================================================================== */
window.openSessionsModal = async function () {
    const modal = document.getElementById("sessions-vault-modal");
    if (modal) modal.style.display = "flex";
    await refreshSessionsModalUI();
};
...
window.handleSessionsModalBackdropClick = function (event) {
    if (event.target && event.target.id === "sessions-vault-modal") {
        closeSessionsModal();
    }
};

window.refreshSessionsModalUI = async function () {
    const statusDot = document.getElementById("vault-status-dot");
    const statusEl = document.getElementById("vault-li-status");
    const accountBox = document.getElementById("vault-li-account-box");
    const nameEl = document.getElementById("vault-li-account-name");
    const descEl = document.getElementById("vault-status-description");
    ...
    try {
        ...
        const isLiValid = !!(data.linkedin && data.linkedin.valid);
        const accountName = data.linkedin?.account_name || "LinkedIn Member";
        ...
        if (isLiValid) {
            ...
            if (statusEl) {
                statusEl.innerText = "ACTIVE & CONNECTED";
                statusEl.style.color = "#34d399";
            }
            if (accountBox) {
            ...
            if (descEl) {
                descEl.innerText = "Your desktop browser session is securely connected and active. LinkedIn OSINT queries will use your session safely.";
            }
            if (launchBtnText) {
                launchBtnText.innerText = "🔄 Re-authenticate / Refresh Session";
            }
            updateBrowserHeaderStatus({ valid: true, account_name: accountName });
        } else {
            if (statusDot) {
            ...
            if (descEl) {
                descEl.innerText = "Connect your desktop browser once to enable private LinkedIn profile intelligence without getting flagged.";
            }
            if (launchBtnText) {
                launchBtnText.innerText = "🚀 Launch Browser & Connect";
            }
            updateBrowserHeaderStatus({ valid: false });
        }
    } catch (e) {
        console.warn("Could not check session status:", e);
        if (statusEl) {
            statusEl.innerText = "STATUS CHECK FAILED";
            statusEl.style.color = "#f87171";
        }
        updateBrowserHeaderStatus({ valid: false });
    }
...
function setVaultFeedback(msg, isSuccess) {
    const el = document.getElementById("vault-modal-feedback");
    if (!el) return;
    el.innerText = msg;
    el.style.display = "block";
    if (isSuccess) {
...
window.handleCookieInputAutoDetect = function (el) {
    if (!el || !el.value) return;
    const val = el.value.trim();

    // Check if user pasted full Cookie header containing JSESSIONID
    if (val.includes("JSESSIONID=") || (val.includes("li_at=") && val.includes(";"))) {
        let li_at = "";
        let jsessionid = "";

        const mLi = val.match(/(?:^|;\s*|\s+)li_at=([^;,\s]+)/);
        if (mLi) li_at = mLi[1].replace(/^["']|["']$/g, "");

        const mJs = val.match(/(?:^|;\s*|\s+)JSESSIONID="?([^;,\s"]+)"?/);
        if (mJs) jsessionid = mJs[1].replace(/^["']|["']$/g, "");

        if (li_at) el.value = li_at;
        const jsInput = document.getElementById("vault-li-jsession-input");
        if (jsInput && jsessionid) {
            jsInput.value = jsessionid;
            setVaultFeedback("✓ Auto-extracted li_at and JSESSIONID from pasted cookie header!", true);
        }
    }
};

window.checkLinkedInCookieLive = async function (showAlerts = true) {
    const statusEl = document.getElementById("vault-li-status");
    const accountBox = document.getElementById("vault-li-account-box");
    const nameEl = document.getElementById("vault-li-account-name");
    const subEl = document.getElementById("vault-li-account-sub");
    const btnCheck = document.getElementById("btn-vault-check-li");
    const inputLi = document.getElementById("vault-li-cookie-input");
    const inputJs = document.getElementById("vault-li-jsession-input");

    const liVal = inputLi ? inputLi.value.trim() : "";
    const jsVal = inputJs ? inputJs.value.trim() : "";

    if (btnCheck) btnCheck.innerText = "Checking...";
    if (statusEl) {
        statusEl.innerText = "TESTING LIVE VOYAGER API...";
        statusEl.style.background = "rgba(245, 158, 11, 0.15)";
        statusEl.style.color = "#fbbf24";
        statusEl.style.borderColor = "#f59e0b";
    }
    setVaultFeedback("Connecting to LinkedIn Voyager API...", true);

    try {
        const body = {};
        if (liVal) body.cookie = liVal;
        if (jsVal) body.jsessionid = jsVal;

        const res = await fetch("/api/sessions/verify_linkedin", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (data.valid) {
            if (statusEl) {
                statusEl.innerText = "VALID & AUTHENTICATED";
            ...
            if (accountBox) {
                accountBox.style.display = "block";
                if (nameEl) nameEl.innerText = data.account_name || "LinkedIn Member";
                if (subEl) subEl.innerText = (data.headline ? `${data.headline} • ` : "") + "Voyager API Authenticated";
            }
            // Update top header cookie logo to GREEN
            updateCookieHeaderStatus({ valid: true, account_name: data.account_name });
            setVaultFeedback(`✓ Session Active & Verified: ${data.account_name}`, true);
            if (showAlerts && typeof showToast === "function") {
                showToast(`LinkedIn Session Active: ${data.account_name}`, "success");
            }
        } else {
            if (statusEl) {
                statusEl.innerText = data.configured ? "! EXPIRED / INVALID" : "! NOT CONFIGURED";
                statusEl.style.background = "rgba(239, 68, 68, 0.2)";
                statusEl.style.color = "#f87171";
                statusEl.style.borderColor = "#ef4444";
            }
            if (accountBox) accountBox.style.display = "none";
            // Update top header cookie logo to RED with '!'
            updateCookieHeaderStatus({ valid: false });
            setVaultFeedback("! " + (data.message || "LinkedIn cookie check failed."), false);
            if (showAlerts && typeof showToast === "function") {
                showToast(data.message || "LinkedIn cookie check failed.", "error");
            }
        }
    } catch (e) {
        console.error("Live LinkedIn verify error:", e);
        if (statusEl) {
        ...
        updateCookieHeaderStatus({ valid: false });
        setVaultFeedback("! Network check failed: " + e.message, false);
    } finally {
        if (btnCheck) btnCheck.innerText = "Check Live";
    }
};

window.saveLinkedInCookieFromModal = async function () {
    const inputLi = document.getElementById("vault-li-cookie-input");
    const inputJs = document.getElementById("vault-li-jsession-input");
    const liVal = inputLi ? inputLi.value.trim() : "";
    const jsVal = inputJs ? inputJs.value.trim() : "";
    ...
    if (!liVal) {
        setVaultFeedback("! Please paste your LinkedIn 'li_at' cookie string first.", false);
        if (typeof showToast === "function") {
            showToast("Please paste your LinkedIn 'li_at' cookie string first.", "warning");
        }
        if (inputLi) inputLi.focus();
        return;
    }

    const btnSave = document.getElementById("btn-vault-save-li");
    if (btnSave) btnSave.innerText = "Saving & Testing...";
    setVaultFeedback("Saving to .env and testing credentials live...", true);

    try {
        const body = { cookie: liVal };
        if (jsVal) body.jsessionid = jsVal;

        const res = await fetch("/api/sessions/save_linkedin", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body)
        });
        ...
        if (data.success) {
            if (inputLi) inputLi.value = "";
            if (inputJs) inputJs.value = "";
            await refreshSessionsModalUI();
            const isVal = data.verification?.valid;
            updateCookieHeaderStatus({ valid: isVal, account_name: data.verification?.account_name });
            const verMsg = data.verification?.message || "Saved to .env";
            setVaultFeedback(`✓ LinkedIn session saved to .env! (${verMsg})`, isVal);
            if (typeof showToast === "function") {
                showToast("Cookie saved: " + verMsg, isVal ? "success" : "warning");
            }
        } else {
            setVaultFeedback("! " + (data.message || "Could not save cookie."), false);
            if (typeof showToast === "function") {
                showToast(data.message || "Could not save cookie.", "error");
            }
        }
    } catch (e) {
        console.error("Error saving cookie:", e);
        setVaultFeedback("! Error: " + e.message, false);
    } finally {
        if (btnSave) btnSave.innerText = "Save to .env";
    }
};

window.updateBrowserHeaderStatus = async function (customStatus) {
    const btn = document.getElementById("btn-header-connect-browser");
    const text = document.getElementById("browser-btn-text");
    ...
    if (customStatus !== undefined && customStatus !== null) {
        if (typeof customStatus === "boolean") {
            isGood = customStatus;
        } else if (typeof customStatus === "object") {
            isGood = !!customStatus.valid || customStatus.overall_status === "good";
            accountName = customStatus.account_name;
        }
    } else {
        try {
            const res = await fetch("/api/sessions/status");
            if (res.ok) {
                const data = await res.json();
                isGood = !!(data.linkedin && data.linkedin.valid);
                accountName = data.linkedin?.account_name;
            }
        } catch (e) {
            console.warn("Could not check session status:", e);
        }
    ...
    if (isGood) {
        ...
        if (text) {
            text.innerText = accountName ? ("CONNECTED: " + accountName.split(" ")[0].toUpperCase()) : "BROWSER: CONNECTED";
        }
        btn.title = accountName ? `LinkedIn Session Active: ${accountName}` : "LinkedIn Browser Session Connected & Active";
    } else {
        ...
        if (text) {
            text.innerText = "CONNECT BROWSER";
        }
        btn.title = "Connect Local Desktop Browser Session for LinkedIn Intelligence";
    }
};

// Aliases for compatibility
window.updateCookieHeaderStatus = window.updateBrowserHeaderStatus;
window.updateSessionsHeaderStatusBadge = window.updateBrowserHeaderStatus;
window.updateSessionsHeaderStatus = window.updateBrowserHeaderStatus;

window.openSessionsModal = async function () {
    console.log("[Browser Session] Opening connect browser modal...");
    const modal = document.getElementById("sessions-vault-modal");
    if (modal) {
        modal.style.display = "flex";
...
window.connectDesktopBrowser = async function () {
    console.log("[Browser Connect] Launching dedicated browser session...");
    const modal = document.getElementById("sessions-vault-modal");
    if (!modal || modal.style.display === "none") {
        window.openSessionsModal();
    }

    const feedback = document.getElementById("vault-modal-feedback");
    const launchBtn = document.getElementById("btn-vault-launch-browser");
    const launchBtnText = document.getElementById("btn-launch-text");
    const headerBtnText = document.getElementById("browser-btn-text");

    if (launchBtn) {
        launchBtn.disabled = true;
    }
    if (launchBtnText) {
        launchBtnText.innerText = "Opening Browser Window...";
    }
    if (headerBtnText) {
        headerBtnText.innerText = "LAUNCHING...";
    }
    if (feedback) {
        ...
        feedback.innerHTML = "<strong>Browser Window Opened:</strong> A secure browser window has opened on your desktop. Sign into LinkedIn (via Google or password). BreachSpillover will automatically capture your verified session.";
    }
    ...
    try {
        ...
        let checks = 0;
        // Human-like polling: calm 4000ms interval (no request spamming)
        const pollInterval = setInterval(async () => {
            checks++;
            if (checks > 45) { // 3 minutes timeout
                ...
                if (headerBtnText) headerBtnText.innerText = "CONNECT BROWSER";
                if (feedback) {
                    feedback.innerHTML = "<strong>Timed Out:</strong> Login detection timed out. You can click 'Launch Browser & Connect' to try again.";
                }
                return;
            }
            try {
                const stResp = await fetch("/api/sessions/status");
                const stData = await stResp.json();
                if (stData.linkedin && stData.linkedin.valid) {
                    clearInterval(pollInterval);
                    const acctName = escapeHtml(stData.linkedin.account_name || 'LinkedIn Member');
                    if (feedback) {
                        feedback.style.background = "rgba(16, 185, 129, 0.15)";
                        feedback.style.color = "#34d399";
                        feedback.style.border = "1px solid #10b981";
                        feedback.innerHTML = `<strong>Connected &amp; Verified:</strong> Successfully authenticated as <strong>${acctName}</strong>!`;
                    }
                    if (launchBtn) {
                        launchBtn.disabled = false;
                    }
                    if (launchBtnText) {
                        launchBtnText.innerText = "✓ Connected Successfully!";
                    }
                    if (headerBtnText) {
                        headerBtnText.innerText = "CONNECTED";
                    }
                    updateBrowserHeaderStatus({ valid: true, account_name: stData.linkedin.account_name });
                    if (typeof window.refreshSessionsModalUI === "function") {
                        await window.refreshSessionsModalUI();
                    }
                    if (typeof showToast === "function") {
                        showToast(`LinkedIn browser session connected: ${stData.linkedin.account_name || 'Active'}`, "success");
                    }
                    // Auto-close modal after 1.8 seconds so user sees visual confirmation
                    setTimeout(() => {
                        closeSessionsModal();
                    }, 1800);
                }
        ...
        if (feedback) {
            ...
            feedback.innerHTML = `<strong>Error:</strong> Failed to launch browser (${e.message}). You can also run <code>python -m backend.interactive_login</code> in your terminal.`;
        }
        if (launchBtn) {
            launchBtn.disabled = false;
        }
...
// Auto-attach direct click listeners once DOM is ready
document.addEventListener("DOMContentLoaded", function () {
    // Initial header status check
    if (typeof window.updateBrowserHeaderStatus === "function") {
        window.updateBrowserHeaderStatus();
    }
    const btnBrowser = document.getElementById("btn-header-connect-browser");
    if (btnBrowser) {
